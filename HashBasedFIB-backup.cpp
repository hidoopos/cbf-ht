...
//树操作 添加子节点
inline void HashBasedFIB::AuxTreeNode::addChild(AuxTreeNode* nChild) {
    if (nChild == NULL)
        return;
...
//树操作 删除子节点
inline void HashBasedFIB::AuxTreeNode::removeChild(AuxTreeNode* nChild) {
    if (nChild == NULL)
        return;
...
void HashBasedFIB::AuxTreeNode::setNonrealNodesInSubtreeTo(
        HashBasedFIB::AuxTreeNode::NodeType type) {
    this->type = type;
...
void HashBasedFIB::insert(const string& input, const ForwardInfo& info) {
    ...
    if (it != mMap.end()) {
        ...
        while (1) {
            ...
            if (end_pos == 0) {
                //表中不存在任何一个该名字的前缀
                nNode->setNonrealNodesInSubtreeTo(AuxTreeNode::Virtual);
                root->addChild(nNode);
                fib_debug("    Non-real entries above are virtual.\n")
                return;
            }
            //查找前一级前缀是否存在于表中
            it = mMap.find(input.substr(0, end_pos));
            if (it != mMap.end()) {
                //前一级前缀为表中的实节点，连接到该节点上
                ...
                if (it->second->type == AuxTreeNode::Virtual) {
                    ...
                    fib_debug("    Non-real entries above are semi-virtual.\n")
                }
                return;
            }
            else {
                //前一级前缀依然不存在于表中，创建半虚节点
                preNode = nNode;
                nNode = new AuxTreeNode;
                nNode->type = AuxTreeNode::SemiVirtual;
